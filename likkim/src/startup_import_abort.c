...
static void startup_import_abort_bg_cont(lv_obj_t* parent)
{
    ...
    lv_obj_set_width(title_label, 380);
    lv_obj_set_pos(title_label, 90, 60);
	set_language_string(title_label, 40, language_table_abort_import);
	...
	set_language_string(describe_label, 24, language_table_sure_to_abort_process);

    lv_obj_t* img_btn_confirm = gui_comm_draw_obj(parent, 40, 598, 400, 80, 0x7FFF, 0xF7E100, 0x8A7D00);
...
{
    printf("%s\n", __func__);
     gui_algo_data_set_pagelocation("startup_import_abort",0);
    p_startup_import_abort = (startup_import_abort_t*)lv_mem_alloc(sizeof(startup_import_abort_t));
...
void startup_import_abort_stop(void)
{
    ...
    lv_mem_free(p_startup_import_abort);
