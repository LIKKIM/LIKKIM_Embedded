...
static void guide_recovery_bg_cont(lv_obj_t* parent)
{
    ...
    lv_obj_set_pos(title, 40, 64);
	set_language_string(title, 32, language_table_what_is_recovery_phrase);
    ...
    lv_obj_set_size(bg_obj, 400, 800);
	...
	set_language_string(label_describe, 24, language_table_recovery_phrase_description);
...
{
    printf("%s\n", __func__);
    gui_algo_data_set_pagelocation("guide_recovery",0);
    p_guide_recovery = (guide_recovery_t*)lv_mem_alloc(sizeof(guide_recovery_t));
